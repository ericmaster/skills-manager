...
- [ ] **Phase 4 — `update` clean + conflict + `--continue`.** [docs/phases/phase-4-update.md](docs/phases/phase-4-update.md)
  Implement the full update flow from [AGENTS.md](AGENTS.md): save-patch → re-resolve to staging → `git apply --3way` → atomic swap, or leave conflict markers and pause. Tests in `tests/update.test.js`.
- [ ] **Phase 5 — `remove <skill>` + `list`.** [docs/phases/phase-5-remove-and-list.md](docs/phases/phase-5-remove-and-list.md)
